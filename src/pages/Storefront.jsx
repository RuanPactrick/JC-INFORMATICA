        ...
        <div className="mt-auto">
          ...
          <div className="text-[11px] md:text-[12px] text-gray-500 font-medium mt-1.5 mb-5 h-[18px]">
            {(product.installmentCount && product.installmentAmount) ? (
              <>ou {product.installmentCount}x de {formatPrice(product.installmentAmount)}</>
            ) : product.installment ? (
...
function CategoryItem({ category }) {
  const catKey = category.slug || category.id
  ...
  return (
    <a href={`#${catKey}`} className="group flex flex-col items-center gap-3 w-[88px] sm:w-[115px] shrink-0 cursor-pointer snap-start">
      ...
      <div className="w-[88px] h-[88px] sm:w-[112px] sm:h-[112px] rounded-full overflow-hidden bg-brand-purple flex items-center justify-center relative shadow-sm transition-transform duration-300 ease-out group-hover:scale-[1.03]">
        ...
        <img
          src={category.image}
          alt={category.name}
          className={`w-full h-full object-cover ${getObjectPosition(catKey)}`}
          loading="lazy"
        />
...
export default function Storefront() {
    ...
    async function loadDynamicData() {
      try {
        const [catsRes, prodsRes] = await Promise.all([
          ...
          fetch(`/api/products${sortOrder !== 'recent' ? `?sort=${sortOrder}` : ''}`)
        ])
        if (catsRes.ok) {
          const dynamicCats = await catsRes.json()
          if (Array.isArray(dynamicCats) && dynamicCats.length > 0) {
            setCategories(dynamicCats)
          }
        }
        if (prodsRes.ok) {
          const dynamicProds = await prodsRes.json()
          if (Array.isArray(dynamicProds) && dynamicProds.length > 0) {
            setProducts(dynamicProds)
          }
        }
      } catch (err) {
        // Fallback silently to static JSON
      }
    }
    loadDynamicData()
  }, [sortOrder])
  ...
  // Featured: products with promotion flag, or fallback to first 4
  const featured = filteredProducts.filter(p => p.promotion)
  const featuredProducts = featured.length > 0 ? featured : filteredProducts.slice(0, 4)
  const allProducts = filteredProducts.slice(0, 8)
          ...
          <div className="flex items-center h-[52px]">
            <div className="flex items-center gap-6 lg:gap-8 text-[13px] font-bold tracking-wide">
              ...
              {categories.slice(0, 8).map(cat => (
                <a key={cat.id || cat.slug} href={`#${cat.slug || cat.id}`} className="hover:text-white transition-colors">
                  {cat.name}
                </a>
              ))}
            </div>
      ...
      <main className="flex-1 py-12 md:py-14">
        ...
        <section className="max-w-container mx-auto px-4 lg:px-8 mb-12 md:mb-16">
          ...
          <div className="flex overflow-x-auto gap-4 md:gap-7 lg:justify-between pb-6 pt-2 -mx-4 px-4 lg:mx-0 lg:px-0 scrollbar-hide snap-x">
            {categories.map(cat => (
               <CategoryItem key={cat.id || cat.slug} category={cat} />
            ))}
          </div>
        </section>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-5 md:mb-6 gap-4">
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              <select 
                ...
                className="bg-white border border-surface-200 text-gray-900 text-[13px] rounded-[6px] focus:ring-2 focus:ring-brand-purple focus:border-brand-purple block py-1.5 px-3 font-semibold outline-none cursor-pointer"
              >
